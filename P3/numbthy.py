...
def runCombinationCheckingMethod(fun, args=[], kwargs = {}):    
    ...
    print 'Method name: ' , fun, "Time elapsed : ", timeSpan
...
if __name__ == '__main__':
    ...
    print isprime(p)
    how_many = 1024 * 1024
    inv_pow_up_to_20 = [large_power_mod(g, p - how_many, p)]
    big_powers = [1]
    base_big_power = large_power_mod(g, how_many, p)
    for e in range(how_many-1): 
        inv_pow_up_to_20.append((inv_pow_up_to_20[-1] * g) % p)
        big_powers.append((big_powers[-1] * base_big_power) % p)
    lhs = [(e, (h * inv_pow_up_to_20[-1 - e]) % p) for e in range(how_many)]
    lhs.sort(key = lambda l:l[1])
    rhs = [(e, big_powers[e]) for e in range(how_many)] 
    rhs.sort(key = lambda r:r[1])
    ...
    rCounter = 0
    lCounter = 0
    while (lCounter < how_many) and (rCounter < how_many):
        assert (lhs[lCounter][1] - h * inv_pow_up_to_20[-1-lhs[lCounter][0]]) % p == 0
        if lhs[lCounter][1] == rhs[rCounter][1]:
            print rhs[rCounter][0] * how_many + lhs[lCounter][0]
            print (large_power_mod(g, rhs[rCounter][0] * how_many + lhs[lCounter][0], p) - h) % p
            break
        if lhs[lCounter][1] < rhs[rCounter][1]:
            lCounter += 1
        else:
